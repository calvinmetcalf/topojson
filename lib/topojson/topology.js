...
var stitch = require("./stitch-poles");
...
module.exports = function(objects, options) {
  ...
  if (options) {
    ...
    if ("property-transform" in options) {
  ...
  each({
    point: function(point) {
      var x1 = point[0],
          ...
          e = system.distance(x1, y1, x / kx + x0, y / ky + y0);
      if (e > emax) {
        emax = e;
      }
      point[0] = x;
  ...
  function line(points, open) {
    ...
    for (; k < n; ++k) {
      ...
      if (p && !linesEqual(p, t)) {
        tInP = t.every(function(line) {
          return p.indexOf(line) >= 0;
        });
        pInT = p.every(function(line) {
          return t.indexOf(line) >= 0;
        });
        if (tInP && !pInT) {
          --k;
        }
